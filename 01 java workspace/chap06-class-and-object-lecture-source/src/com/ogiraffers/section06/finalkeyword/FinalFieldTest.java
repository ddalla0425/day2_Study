...
public class FinalFieldTest {
    ...
    //3. 인스턴스 변수(non-static) : 인스턴스 생성 후 초기화 이후에 값 변경 불가
    //4. 클래스 변수(static) : 프로그램 시작 이후 값 변경 불가
    //5. non-static 메소드 : 메소드 재작성(overriding)불가 ┐
    //6. static 메소드 : 메소드 재작성(overriding) 불가    ├> 아직 안배움
    //7. 클래스 : 상속 불가                              ┘


    //non-static 필드에 final 사용
    //초기 인스턴스 생성 시 기본 값이  0인 필드에 들어가게 되는데 초기화 이후 값 변경이 불가능 하므로
...
//    private  final int nonStaticNum;
    ...
    static {
        STATIC_DOUBLE = 0.5;  // 이 코드를 넣기 전엔 위의 코드에 빨간 에러가 있었다.(WHy? 초기화가 안돼서)
    }

    //public static final 을 상수 필드라고 칭한다.
    //Math.PI; >>> 타고 ctrl누르고 타고 들어가면 이렇게 쓴다는걸 볼 수 있다.
