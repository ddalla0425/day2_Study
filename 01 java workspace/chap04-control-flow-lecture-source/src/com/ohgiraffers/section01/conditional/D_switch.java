...
public class D_switch {
    public void testSimpleSwitchStatement() {
        ...
        switch (op) { // case 옆에 들어가는 건 반드시 리터럴 값이어야 함
            case '+':
                result = first + second; //케이스 명령어는 여러개 서술 가능 ,
                break;            //명령어 끝엔 항상 break; 를 적어줘야함 , 안적으면 미끄러짐
            case '-':
                result = first - second;
                break;
            case '*':
                ...
                break;
            case '/':
                //second(나누는 수)가 0이면 java.lang.ArithmenticException : by zero 발생함
                result = first / second;
                break;
            case '%':
                result = first % second;
    ...
    public void testSwitchVendingMachine() {
        ...
        switch (selectedDrink) {
            ...
            case "핫식스":
                ...
                break;
            default:
                order = "없는 음료"; //case에 해당하는게 없는 상황에 실행
        }
