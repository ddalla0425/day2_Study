...
public class C_ifElseIf {
    ...
    public void testNestedIfElseStatement() {
        ...
        //90점 이상이면 'A', 80점 이상이면 'B', 70점 이상이면 'C', 60점 이상이면 'D', 60점 미만이면 'F'를 학점 등급으로 매겨본다.
        ...
        String grade = ""; //String 타입 쓰고 싶고 현재 초기화할 값이 정해 지지 않았다! -> 빈 문자열로 초기화

        if (point >= 90) {          // if문은 순서대로 진행되니 항상 순서를 지켜줘야 한다
            grade = "A";
        } else if (point >= 80) {
            grade = "B";
        } else if (point >= 70) {
        ...
        if (point % 10 >= 5 && point >= 60 || point == 100) { //point % 10 하면 앞에가 95든 85든 75든 나머지는 앞에 한자리가 날아가고 나머지 한자리가 남는다 &&가 우선순위
            grade += "+";
        }
